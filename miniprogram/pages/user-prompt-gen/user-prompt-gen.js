...
Page({
    ...
    getPromptByJson:function(selectedStyles) {
        ...
        // 查询或更新选项组合的使用次数
        db.collection('promptChoices').where({
            choiceKey: choiceKey
        }).update({
            data: {
            ...
            if (res.stats.updated === 0) {
                ...
                return db.collection('promptChoices').add({
                    data: {
                        choiceKey: choiceKey,
                        count: 1
                    }
    ...
    onLoad(options) {
        ...
        const app = getApp()
        const selectedStyle = app.globalData.selectedStyles // 获取选择的风格
        // add card 
        const styleContents = selectedStyle.map((choice, index) => {
            return this.data.styleGroup[index + 1][choice];
        });
        // 设置标签内容
        this.setData({
            styleLabels: styleContents
        });
    ...
    handleRegenerate: function() {
        // 方法1：重新加载当前页面
        wx.reLaunch({
            url: '/pages/user-prompt-1/user-prompt-1',
            success: () => {
